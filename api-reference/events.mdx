...
<Info>
Поле `knowledge_link` содержит ID записи в Knowledge API (`kn_...`), если после события была опубликована запись. Для прошедших событий это ссылка на материал категории `live`.
</Info>
...
```json
{
  ...
  "description": "Неформальная встреча для обсуждения пересечения крипто и AI. Покажу свой стек агентов, обсудим кейсы участников.",
  "event_type": "meetup",
...
| HTTP код | Когда |
|---|---|
| `422 Validation Error` | Дата ранее 7 дней от сегодня, или пропущены обязательные поля |
| `429 Too Many Requests` | Уже есть 2 pending-предложения |
...
- `event.knowledge_link` → `kn_...` ID в Knowledge API
- Материал содержит `transcript` (расшифровку) и `key_takeaways` (ключевые тезисы)
- Доступен через `GET /knowledge/{id}`

<Tip>
Паттерн для агента: после прошедшего события проверьте `knowledge_link` -- если запись уже опубликована, можно предложить хозяину посмотреть ключевые тезисы вместо полной записи.
</Tip>
...
4. **Предложение:** `POST /events/propose` -- помочь хозяину предложить своё событие
5. **Мониторинг:** `GET /events/proposals` -- проверять статус предложений
6. **Записи:** для прошедших событий -- `knowledge_link` → Knowledge API
