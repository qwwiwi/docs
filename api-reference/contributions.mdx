...
Contributions -- механизм вклада в экосистему EdgeLab. Любой участник на любом тарифе может предложить скилл, use case или урок. Одобренные работы публикуются в разделе Knowledge с указанием автора.
...
При одобрении contribution автоматически публикуется в разделе Knowledge с указанием автора. Все тарифы (Edge, Pro, VIP) могут быть контрибьюторами.

<Info>
**Вознаграждение:** 20% от подписок направляется в Governance Fund, из которого распределяются выплаты контрибьюторам.
</Info>
...
<ParamField body="github_url" type="string" required>
  Ссылка на публичный GitHub-репозиторий с исходным материалом
</ParamField>
...
| `422 Validation Error` | Невалидные данные: пустой title, неизвестный type, невалидный github_url |
...
<ParamField query="per_page" type="integer" default="20">
  Количество на странице (макс. 50)
</ParamField>
...
<CodeGroup>
```bash cURL
curl https://platform.edgelab.su/api/v1/contributions/my?status=approved \
  -H "Authorization: Bearer edgelab_live_abc123"
```
</CodeGroup>
...
|----------|-------|
| `401 Unauthorized` | Невалидный или отсутствующий API-ключ |
| `404 Not Found` | Contribution с указанным ID не найден |

<Note>
**GitHub-only:** Все contributions принимаются исключительно через ссылку на публичный GitHub-репозиторий. Это обеспечивает прозрачность, версионирование и возможность community-ревью.
</Note>
