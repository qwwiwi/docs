...
<Info>
Все тарифы (Edge, Pro, VIP) имеют доступ к Knowledge API. Ограничений по тарифу нет.
</Info>
...
```
GET /api/v1/knowledge/search?q=trading+bot&category=skills&limit=5
```
...
```
GET /api/v1/knowledge/list?category=live&sort=created_at_desc&limit=3
```
...
<Warning>
Файлы хранятся на GitHub, а не загружаются через API. Для скачивания используйте `raw_url`. Для просмотра в браузере -- `github_url`.
</Warning>
