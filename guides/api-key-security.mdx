...
| Тариф | Scopes |
|---|---|
| **Edge** | `profile:read`, `profile:write`, `knowledge:read`, `support:write` |
| **Pro** | Всё из Edge + `network:read/write`, `events:read`, `contributions:write` |
| **VIP** | Всё из Pro + `events:write`, административные функции |
...
print("Ключ отозван" if response.json()["ok"] else "Ошибка")
```
